...
static int CLI_CheckParam(int argc, char** argv, PHY_DRIVE_INFO* pDrvInfo, CLI_CFG *pCfg)
{
    ...
    Log("YUMI CLI %s PhyDrive:%d %s SecureBoot:%d ReserveSpace:%dMB USBCheck:%u FS:%s NonDest:%d",
        op == 0 ? "install" : "update",
    ...
    if (IsVentoyPhyDrive(PhyDrive, pDrvInfo->SizeInBytes, &MBR, &Part2StartSector, &Part2GPTAttr))
    {
        ...
        Log("PhyDrive %d is YUMI Disk ver:%s SecureBoot:%u", pDrvInfo->PhyDrive, pDrvInfo->VentoyVersion, pDrvInfo->SecureBootSupport);
...
static int Ventoy_CLI_NonDestInstall(PHY_DRIVE_INFO* pDrvInfo, CLI_CFG* pCfg)
{
    ...
    {
        Log("YUMI does not support 4k native disk.");
        rc = 1;
        goto out;
    }
...
int VentoyCLIMain(int argc, char** argv)
{
    ...
    else
    {
        if (pDrvInfo->VentoyVersion[0] == 0)
        {
            Log("[ERROR] No YUMI information detected in PhyDrive %d, so can not do update", pDrvInfo->PhyDrive);
            goto end;
        }
