 ...
 * PhyDrive.c
 *
 * Copyright (c) 2023, lance <pendrivelinux.com> 
...
int ClearVentoyFromPhyDrive(HWND hWnd, PHY_DRIVE_INFO *pPhyDrive, char *pDrvLetter)
{
    ...
    *pDrvLetter = 0;
    ...
    Log("Clear successfully finished");
...
End:
    ...
    if (rc == 0)
    {
        ...
        for (i = 0; i < sizeof(DriveLetters) && DriveLetters[i]; i++)
        {
            ...
            Log("%s is part1, already mounted", DriveName);
            state = 1;
        }

        if (state != 1)
        {
            Log("need to mount part1...");
            ...
            {
                Log("Failed to find YUMI volume");
            }
        }
...
int InstallVentoy2FileImage(PHY_DRIVE_INFO *pPhyDrive, int PartStyle)
{
    ...
    Log("Writing image file finished, the file size should be %llu .", data_offset + 512 + ((dataLen + 511) / 512 * 512));
...
End:
    ...
    if (rc == 0)
    {
        ...
        memset(DriveLetters, 0, sizeof(DriveLetters));
        GetLettersBelongPhyDrive(pPhyDrive->PhyDrive, DriveLetters, sizeof(DriveLetters));
        Log("Logical drive letter after write YUMI: <%s>", DriveLetters);
        ...
        for (i = 0; i < sizeof(DriveLetters) && DriveLetters[i]; i++)
        {
            ...
            {
                Log("%s is part2, delete mountpoint", DriveName);
            ...
            else
            {
				...
				// For YUMI
				char ypath[8];
				strcpy_s(ypath, _countof(ypath), DriveName);
				strcat_s(ypath, _countof(ypath), "YUMI");

				if (_mkdir(ypath) == 0)
				{
					Log("YUMI Directory on '%s' was successfully created at '%s'", DriveName, ypath);
				} // End For YUMI
...
int PartitionResizeForVentoy(PHY_DRIVE_INFO *pPhyDrive)
{
    ...
    if (g_CLI_Mode)
    {
        Log("### non-destructive CLI installation successfully finished.");
    }
    else
    {
        //Refresh disk list
        ...
        if (pPhyDrive)
        {
            ...
            {
                Log("After process the version is still invalid");
                goto End;
            }

            Log("### non-destructive installation successfully finished <%s>", pPhyDrive->VentoyVersion);
        }
        else
        {
            Log("### non-destructive installation successfully finished <not found>");
        }
...
int UpdateVentoy2PhyDrive(PHY_DRIVE_INFO *pPhyDrive, int TryId)
{
	...
	else
	{
		...
		for (i = (int)strlen(DriveLetters); i > 0; i--)
		{
			...
			if (IsVentoyLogicalDrive(DriveName[0]))
			{
				Log("%s is YUMI logical drive", DriveName);
				bRet = DeleteVolumeMountPointA(DriveName);
