...
int ParseCmdLineOption(LPSTR lpCmdLine)
{
    ...
    if (lpCmdLine && lpCmdLine[0])
    {
		...
		safe_sprintf(PhyDri, "%s", lpCmdLine); // For YUMI - Copy Commandline Argument to PhyDri.
    }
    ...
    strcat_s(cfgfile, sizeof(cfgfile), "\\YUMI2USB.ini");
...
BOOL IsVentoyPhyDrive(int PhyDrive, UINT64 SizeBytes, MBR_HEAD *pMBR, UINT64 *Part2StartSector, UINT64 *GptPart2Attr)
{
	...
	if (MBR.PartTbl[0].FsFlag == 0xEE)
	{
		...
		if (memcmp(pGpt->PartTbl[1].Name, L"VTOYEFI", 7 * 2))
		{
			...
			{
				Log("Invalid efi part name <%S>", pGpt->PartTbl[1].Name);
			}
			else
			{
				Log("Invalid efi part name <null>");
			}
    ...
    Log("PhysicalDrive%d is YUMI disk", PhyDrive);
    return TRUE;
}
...
static int FilterPhysicalDrive(PHY_DRIVE_INFO *pDriveList, DWORD DriveCount)
{
    ...
    for (i = 0; i < DriveCount; i++)
    {
        ...
        if (IsVentoyPhyDrive(CurDrive->PhyDrive, CurDrive->SizeInBytes, &MBR, &Part2StartSector, &Part2GPTAttr))
        {
            ...
            Log("PhyDrive %d is YUMI Disk ver:%s SecureBoot:%u", CurDrive->PhyDrive, CurDrive->VentoyVersion, CurDrive->SecureBootSupport);
